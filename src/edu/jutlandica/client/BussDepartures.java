...
public class BussDepartures {
	private static ArrayList<BussDepartures> departures;
	...
	private BussDepartures(String line, String destination, int departureHour, int departureMinute, int arrivalHour, int arrivalMinute) {
		this.line = line;
	...
	private static void init() {
		departures = new ArrayList<BussDepartures>();
		departures.add(new BussDepartures("9", DANMARK, 16, 23, 16, 43));
		departures.add(new BussDepartures("32", TYSKLAND, 18, 14, 18, 35));	
	}
	
	public static BussDepartures getNextDeparture(int hour, int minute, String destination) {
		if (departures == null) init();
		
		for (BussDepartures d : departures) {
